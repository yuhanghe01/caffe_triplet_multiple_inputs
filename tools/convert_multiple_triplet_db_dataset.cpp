...
int main(int argc, char** argv) {
  ...
  CHECK( multiple_triplet_list_name.size() > 0 ) << "the triplet_list_name param should be specified!";
  ...
  while( std::getline( infile, line_tmp) ){
    std::vector< std::string > split_str_rst;
    split_str( line_tmp, ' ', split_str_rst );
    CHECK( split_str_rst.size() > 5 ) << "each line should at least contain 5 elements, including 3 images and 2 slice parameter!";
    int slice_anchor_pos = 0;
    int slice_pos_neg = 0;
    std::stringstream ss_anchor_pos;
    std::stringstream ss_pos_neg;
    ss_anchor_pos << split_str_rst[ split_str_rst.size() - 2 ];
    ss_pos_neg << split_str_rst[ split_str_rst.size() - 1 ];
    ss_anchor_pos >> slice_anchor_pos;
    ss_pos_neg >> slice_pos_neg;
    //LOG( INFO ) << "slice_anchor_pos = " << slice_anchor_pos;
    //LOG( INFO ) << "slice_pos_neg = " << slice_pos_neg;
    struct MultipleTripletPair trip_pair_tmp;
    for(int i = 0; i < slice_anchor_pos; ++i ){
      trip_pair_tmp.anchor_vec.push_back( split_str_rst[i]);
    }
    for( int i = slice_anchor_pos; i < slice_pos_neg; ++i ){
      trip_pair_tmp.pos_vec.push_back( split_str_rst[i] );
    }
    for( int i = slice_pos_neg; i < split_str_rst.size() - 2; ++i ){
      trip_pair_tmp.neg_vec.push_back( split_str_rst[i] );
    }
  ...
  //struct TripletMultipleDatum mult_triplet_datum;
  //TripletMultipleDatum mult_triplet_datum;
  int count = 0;
  //int data_size = 0;
  //bool data_size_initialized = false;

  for( int line_id = 0; line_id < mult_trip_list_total.size(); ++line_id ) {
    bool status;
    ...
    if (encoded && !enc.size()) {
      ...
      string fn = mult_trip_list_total[line_id].anchor_vec[0];
      size_t p = fn.rfind('.');
      if ( p == fn.npos )
    ...
    status = ReadMultipleTripletImagesToMultipleTripletDatum( mult_trip_list_total[line_id], resize_height, resize_width, is_color, enc, &mult_triplet_datum);
    //LOG( INFO ) << "finished gen db once";
    if (status == false) continue;
    //LOG( INFO ) << mult_triplet_datum.data_anchor_size(); 
    //LOG( INFO ) << mult_triplet_datum.data_pos_size(); 
    //LOG( INFO ) << mult_triplet_datum.data_neg_size(); 
    /*
    if (check_size) {
    ...
    //}
    // sequential
    string key_str = caffe::format_int(line_id, 8) + "_" + mult_trip_list_total[line_id].anchor_vec[0];

    // Put in db
    std::string out;
    CHECK( mult_triplet_datum.SerializeToString( &out ) );
    txn->Put( key_str, out );
