...
void NaiveTripletLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  Dtype loss = 0;
  Dtype accuracy = 0;
  //since we use cosine distance here, it means the distance between any to query images are between [-1, 1]. the larger of the distance, the lower value it holds, thus the neg_sim[i] is often smaller than pos_sim[i] in most cases. That why we first minus pos_sim and then plus neg_sim in calculating the distance.
